...
| M7 design | Lease single-writer trên shared filesystem đã probe; backup/restore bất biến và fail-closed; subprocess E2E; audit bảo mật offline; runbook vận hành. Giữ nguyên v1/v2, đúng hai cổng và đăng thủ công. | Implementation in progress; final C2C pending | Spec self-review; `git diff --check` | `docs: design M7 hardening workflow` |
| M7 plan | Sáu lát test-first: lease/storage, toàn bộ CLI mutation, backup/restore, process E2E, security/doctor/runbook và acceptance/C2C. B1–B4 closeout ở `docs/superpowers/plans/2026-09-16-m7-closeout-steps.md`. | Tasks 1–4 complete; Task 5 in progress; Task 6 pending | Plan/spec consistency; `git diff --check` | `docs: design M7 hardening workflow` |
...
| M7.5 B3 | Runbook lệnh thật cho doctor, busy/lease recovery, backup/restore, security và Git audit; cập nhật AGENTS và `/pyh` giữ hai cổng duyệt và đăng thủ công. | Complete; B4 acceptance/C2C còn mở | 9 contract/skill tests; full Python 918 passed, 2 skipped; Ruff; `git diff --check` | `docs: add M7 operational runbook` |
