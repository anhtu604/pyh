...
| M7.5 B1 | Audit project offline: CLI `security-audit` chỉ in rule/path tương đối/lý do, phát hiện link, binding backup hỏng, credential field và media ở path Git chưa ignore; read-only, không in secret. | Complete; B2 doctor và B3 runbook còn mở | 60 focused passed, 1 skipped; full Python 907 passed, 2 skipped; Ruff; `git diff --check` | `feat: add offline project security audit` |
