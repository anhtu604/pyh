...
| Evidence-first editorial exploration Task 1 | Thêm hai main state orientation: `topic_selected → orientation_research_in_progress → awaiting_editorial_direction → author_brief_ready`; bắt buộc `orientation/scope.yaml`, `orientation/completed/<run_id>.yaml`, cho phép cả hai state vào/ra `blocked`, giữ nguyên `packaged`, hai gate và đăng thủ công. | complete | `python -m pytest tests/domain/test_state_graph.py -q` (93 passed); `python -m ruff check src tests` | `feat: add orientation workflow states` |
