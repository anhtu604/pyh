...
| M7.5 B1 | Audit project offline: CLI `security-audit` chỉ in rule/path tương đối/lý do, phát hiện link, binding backup hỏng, credential field và media ở path Git chưa ignore; read-only, không in secret. | Complete; B3 runbook còn mở | 60 focused passed, 1 skipped; full Python 907 passed, 2 skipped; Ruff; `git diff --check` | `feat: add offline project security audit` |
| M7.5 B2 | `healthvideo doctor --project <project>` kiểm tra exclusive-create và same-directory rename bằng file tạm duy nhất trong chính filesystem của project; chỉ dọn file còn đúng marker của probe, báo FAIL và exit 1 nếu thiếu khả năng hoặc dọn chưa xong. | Complete; B3 runbook còn mở | 53 focused passed; full Python 914 passed, 2 skipped; Ruff; `git diff --check` | `feat: probe project filesystem capabilities` |
