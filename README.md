...
M6.5 đã triển khai và qua acceptance, chờ C2C review: storyboard `m6_5_v1`
...
| M6.4 logo | Logo PHY decorative được khai báo `FLOURISH`/`brand`, có hash và rights ledger; cổng duyệt kiểm tra role/kind/file và đưa bytes logo vào hash duyệt. | Complete; included in C2C implementation review iteration 2 DONE | 83 domain/workflow tests; schema export; Ruff; `git diff --check` | `feat: register declared PHY logo for outro` |
...
| M6.5 plan | Năm lát test-first cho contract, medical packet, chart render, production QA và E2E; lát sáu acceptance/C2C. | Implemented Task 1–6; acceptance passed; chờ C2C review DONE | Plan/spec consistency; Ruff; `git diff --check` | `docs: plan M6.5 visual budget implementation` |
